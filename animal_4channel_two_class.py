...
class animal(Dataset):
        ...
        for dir in root_dir:
            ...
            for c in classes:
                for f in os.walk(os.path.join(dir , c)):
                     self.data_files = self.data_files + [(os.path.join(f[0],path),c) for path in f[2] if 'rgb_crop' in path]
    ...
    def __getitem__(self, index):
        ...
        dis_name = img_path.split('/')[-1].split('rgb')[0] +'d.png'
...
train_dir = '/scratch/salman/inference_noise_profile/habib_code/images/train'
test_dir = '/scratch/salman/inference_noise_profile/habib_code/images/val'


animal_transforms = transforms.Compose([  # Compose makes it possible to have many transforms
        animal_scale(),
        transforms.ToTensor()
    ])
trainloader_set = animal(root_dir=[train_dir], transform=animal_transforms)
test_set = animal(root_dir=[test_dir], transform=animal_transforms)
